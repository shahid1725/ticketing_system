...
def tripvoucheradd(request):
    ...
    return render(request,'Sales/Trip_voucher/VoucherAdd.html', {
        'customers': customers,
...
def tripvoucheredit(request,voucher_id):
    ...
    bank_accounts = BankAccount.objects.all()
    meals = MealPlans.objects.all()
    return render(request,'Sales/Trip_voucher/VoucherEdit.html',{
        'voucher': voucher,
        'customers': customers,
        'sales_persons': sales_persons,
        'destination': destination,
...
def tripvoucherlist(request):
    ...
    return render(request,'Sales/Trip_voucher/VoucherList.html',{'voucher': voucher, 'query': query})

def tripvoucherdetail(request,voucher_id):
    voucher = get_object_or_404(TripVoucher, id=voucher_id)
    context = {
        'voucher': voucher
    }
    return render(request,'Sales/Trip_voucher/VoucherDetail.html',context)


@login_required(login_url='admin_loginview')
def tripvoucherdelete(request, voucher_id):
    voucher = get_object_or_404(TripVoucher, id=voucher_id)
    if request.method == "POST":
        voucher.delete()
...
@login_required(login_url='admin_loginview')
def trip_voucher_record_payment(request, id):
    ...
    if request.method == 'POST':
        ...
        return redirect('tripvoucherdetail',  voucher_id=id)

    return render(request, 'Sales/Trip_voucher/VoucherDetail.html', {'voucher': voucher})




def trip_send_voucher_email(request, voucher_id):
    voucher = get_object_or_404(TripVoucher, id=voucher_id)
...
def trip_voucher_email_template(request,voucher_id):
    voucher = get_object_or_404(TripVoucher, id=voucher_id)
...
def send_whatsapp_voucher_message(request, trip_voucher_id):
    ...
    return redirect('tripvoucher')  # Adjust this to your actual trip voucher list view name

def trip_voucher_whatsapp(request, trip_voucher_id):
    trip_voucher = get_object_or_404(TripVoucher, id=trip_voucher_id)
...
def send_whatsapp_postpone_trip_voucher(voucher_id, new_date, new_destination_name=None):
    voucher = get_object_or_404(TripVoucher, id=voucher_id)
...
def send_whatsapp_postpone_trip_voucher_message(request,voucher_id):
    voucher = get_object_or_404(TripVoucher, id=voucher_id)
    ...
    send_whatsapp_postpone_trip_voucher(voucher_id, new_date)
...
def postpone_trip_voucher(request, voucher_id):
    voucher = get_object_or_404(TripVoucher, id=voucher_id)
    ...
    if request.method == "POST":
        try:
            ...
            send_whatsapp_postpone_trip_voucher(
                voucher.id, 
    ...
    return render(request, 'Postpone/postpone_trip_voucher.html', {
        'voucher': voucher,
        'customers': customers,
        'sales_persons': sales_persons,
...
def send_whatsapp_cancel_trip_voucher_message(request, voucher_id, cancellation_date):
    voucher = get_object_or_404(TripVoucher, id=voucher_id)
...
def cancel_trip_voucher(request, id):
    voucher = get_object_or_404(TripVoucher, id=id)
    ...
    if request.method == "POST":
        try:
            ...
            send_whatsapp_cancel_trip_voucher_message(request, voucher.id, voucher.cancellation_date)
