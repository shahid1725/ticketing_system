...
def salevoucher(request):
    ...
    return render(request,'Sales/Sale_voucher/Sale_voucher.html',{'voucher': voucher, 'query': query})
...
def salevoucheradd(request):
    if request.method == "POST":
        try:
            ...
            messages.error(request, 'Failed to create voucher due to invalid data')
...
@login_required(login_url='admin_loginview')
def salevoucherview(request,voucher_id):
    voucher = get_object_or_404(Voucher, id=voucher_id)
    context = {
        'voucher': voucher
    }
    return render(request,'Sales/Sale_voucher/Sale_voucher_view.html',context)

@login_required(login_url='admin_loginview')
def salevoucheredit(request, voucher_id):
    voucher = get_object_or_404(Voucher, id=voucher_id)
...
@login_required(login_url='admin_loginview')
def salevoucherdelete(request, voucher_id):
    voucher = get_object_or_404(Voucher, id=voucher_id)
    if request.method == "POST":
        voucher.delete()
        messages.success(request, 'Voucher deleted successfully.')
        return redirect('salevoucher')  # Adjust this redirect to your actual voucher list view name
    
#--------------------------------------- Record Payment ---------------------------------------

@login_required(login_url='admin_loginview')
def voucher_record_payment(request, id):
    ...
    if request.method == 'POST':
        ...
        return redirect('salevoucherview',  voucher_id=id)

    return render(request, 'voucher/Viewvouchers.html', {'voucher': voucher})

#---------------------------------------- Voucher Email message -----------------------------------------

def send_voucher_email(request, voucher_id):
    voucher = get_object_or_404(Voucher, id=voucher_id)
    ...
    viewcolor_link = f'http://127.0.0.1:8000/voucher_emails/{voucher_id}/'
...
def send_stay_whatsapp_voucher_message(request, voucher_id):
...
def voucher_whatsapp(request,voucher_id):
    voucher = get_object_or_404(Voucher, id=voucher_id)
    return render(request,'whatsapp_templates/voucher_whattsapp.html', {'voucher':voucher})



#------------------------------------- Postpone Voucher message -------------------------------------------


# def send_voucher_whatsapp_postpone_voucher(customer_display_name, resort_name, checkin_date, checkin_time, voucher_id, mobile):
...
#     send_voucher_whatsapp_postpone_voucher(customer_display_name, resort_name, checkin_date, checkin_time, voucher_id, mobile)
#     messages.success(request, 'WhatsApp message sent successfully.')

#     return redirect('salevoucher')

#---------------------------------------------- Postpone Voucher ---------------------------------------
# def postpone_voucher(request, voucher_id):
#     voucher = get_object_or_404(Voucher, id=voucher_id)
...
#             voucher.save()
#             messages.success(request, 'Voucher updated successfully')
#             return redirect('customerview',id=voucher.customer.id)
#         except InvalidOperation:
#             messages.error(request, 'Failed to update voucher due to invalid data')

#         return redirect('postpone_voucher', voucher_id=voucher_id)
...
#     return render(request, 'Postpone/postpone_voucher.html', {
#         'voucher': voucher,
#         'customers': customers,
#         'sales_persons': sales_persons,
#         'resorts': resorts,
...
#     return render(request, 'Sales/Customers/Customers_View.html', {'voucher': voucher})
...
#------------------------------- New Postpone voucher ------------------------------------


def send_whatsapp_postpone_voucher(voucher_id, new_date, new_resort_name=None):
    voucher = get_object_or_404(Voucher, id=voucher_id)
...
def send_whatsapp_postpone_voucher_message(request, voucher_id):
    voucher = get_object_or_404(Voucher, id=voucher_id)
    ...
    send_whatsapp_postpone_voucher(voucher_id, new_date)
...
def postpone_voucher(request, voucher_id):
    ...
    if request.method == "POST":
        try:
            ...
            send_whatsapp_postpone_voucher(
                voucher.id, 
...
def send_whatsapp_cancel_voucher_message(request, voucher_id, cancellation_date):
    voucher = get_object_or_404(Voucher, id=voucher_id)
...
def cancelvoucher(request, voucher_id):
    voucher = get_object_or_404(Voucher, id=voucher_id)
    ...
    if request.method == "POST":
        try:
            ...
            send_whatsapp_cancel_voucher_message(request, voucher.id, voucher.cancellation_date)
