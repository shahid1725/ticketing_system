...
def tripinvoiceadd(request):
    ...
    return render(request, 'Sales/Trip_invoice/InvoiceAdd.html', {
        'customers': customers,
...
@login_required(login_url='admin_loginview')
def tripinvoiceedit(request,invoice_id):
    ...
    return render(request,'Sales/Trip_invoice/InvoiceEdit.html',{
        'invoice': invoice,
        'customers': customers,
        'sales_persons': sales_persons,
        'bank_accounts': bank_accounts,
...
def tripinvoicelist(request):
    ...
    return render(request,'Sales/Trip_invoice/InvoiceList.html',{'invoices': invoices, 'query': query})


@login_required(login_url='admin_loginview')
def tripinvoicedetail(request,invoice_id):
    invoice = get_object_or_404(TripInvoice, id=invoice_id)
    context = {
        'invoice': invoice
    }
    return render(request,'Sales/Trip_invoice/InvoiceDetail.html',context)


@login_required(login_url='admin_loginview')
def tripinvoicedelete(request, invoice_id):
    invoice = get_object_or_404(TripInvoice, id=invoice_id)
    if request.method == "POST":
        invoice.delete()
...
@login_required(login_url='admin_loginview')
def tripinvoice_record_payment(request, id):
    ...
    if request.method == 'POST':
        ...
        return redirect('tripinvoicedetail', invoice_id=id)

    return render(request, 'Sales/Trip_invoice/InvoiceDetail.html', {'invoice': invoice})


#---------------------------------------- trip Invoice Email message -------------------------------------

def tripsend_invoice_email(request, invoice_id):
    invoice = get_object_or_404(TripInvoice, id=invoice_id)
...
def trip_invoice_email(request, invoice_id):
...
def send_whatsapp_invoice_message(request, trip_invoice_id):
    ...
    return redirect('tripinvoice')  # Adjust this to your actual trip invoice list view name

def trip_invoice_whatsapp(request, trip_invoice_id):
    trip_invoice = get_object_or_404(TripInvoice, id=trip_invoice_id)
...
def send_whatsapp_postpone_trip_invoice(invoice_id, new_date, new_destination_name=None):
    invoice = get_object_or_404(TripInvoice, id=invoice_id)
...
def send_whatsapp_postpone_trip_invoice_message(request,invoice_id):
    invoice = get_object_or_404(TripInvoice, id=invoice_id)
    ...
    send_whatsapp_postpone_trip_invoice(invoice_id, new_date)
...
def postpone_trip_invoice(request, invoice_id):
    invoice = get_object_or_404(TripInvoice, id=invoice_id)
    ...
    if request.method == "POST":
        try:
            ...
            send_whatsapp_postpone_trip_invoice(
                invoice.id, 
    ...
    return render(request, 'Postpone/postpone_trip_invoice.html', {
        'invoice': invoice,
        'customers': customers,
        'sales_persons': sales_persons,
...
def send_whatsapp_cancel_trip_invoice_message(request, invoice_id, cancellation_date):
    invoice = get_object_or_404(TripInvoice, id=invoice_id)
...
def cancel_trip_invoice(request, id):
    invoice = get_object_or_404(TripInvoice, id=id)
    ...
    if request.method == "POST":
        try:
            ...
            send_whatsapp_cancel_trip_invoice_message(request, invoice.id, invoice.cancellation_date)
