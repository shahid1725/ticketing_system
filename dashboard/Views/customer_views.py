...
def customerlist(request):
    ...
    return render(request,'Sales/Customers/Customers_List.html', {'customers':customers,'has_customers':has_customers,
                                                                  'query': query})
@login_required(login_url='admin_loginview')
def customerview(request,id):
    ...
    return render(request,'Sales/Customers/Customers_View.html',context)
...
def customeradd(request):
    if request.method == 'POST':
        ...
        if Customers.objects.filter(contact_number=contact_number).exists():
            ...
            return render(request,'Sales/Customers/Customers_Add.html')
...
def customeredit(request, customer_id):
    ...
    if request.method == 'POST':
        ...
        if Customers.objects.filter(contact_number=customer.contact_number).exclude(id=customer.id).exists():
            messages.error(request,'A customer with this contact number already exists')
            return render(request,'Sales/Customers/Customers_Edit.html',{'customer':customer})
        ...
        if len(customer.contact_number) < 10:
            messages.error(request,'Contact number must be at least 10 numbers long')
            return render(request,'Sales/Customers/Customers_Edit.html')
