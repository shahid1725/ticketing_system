...
def ownresortlist(request):
    ...
    return render(request,'Sales/Own_Resort/Own_resort_list.html',{'ownresort':ownresort, 'has_ownresort':has_ownresort})
...
def ownresortadd(request):
    if request.method=='POST':
        ...
        try:
            ...
            return redirect('ownresortlist')
        except Exception as e:
            return HttpResponse(f"An error occurred: {str(e)}")
    return render(request,'Sales/Own_Resort/Own_Resort_Add.html')

@login_required(login_url='admin_loginview')
def ownresortedit(request,id):
    ...
    if request.method=='POST':
        ...
        if OwnResorts.objects.filter(ownresort_mobile=ownresorts.ownresort_mobile).exclude(id=ownresorts.id).exists():
            messages.error(request,'A ownresort with this contact number already exists')
            return render(request,'Sales/Own_Resort/Own_Resort_Edit.html',{'ownresorts':ownresorts})
...
@login_required(login_url='admin_loginview')
def ownresortdelete(request,id):
    ownresorts=get_object_or_404(OwnResorts,id=id)
    if request.method=='POST':
        ownresorts.delete()
