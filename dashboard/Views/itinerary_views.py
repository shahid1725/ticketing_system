...
def itinerary_add(request):
    if request.method == "POST":
      ...
      #           setattr(itinerary, f'extra_activities{i}', activity)
        ...
        return redirect('listitinerary')

    customers = Customers.objects.all()
    sales_persons = CustomUser.objects.all()
    

    return render(request, 'Sales/Itinerary/Itinerary_add.html', {
        'customers': customers,
...
def itinerary_list(request):
    ...
    return render(request, 'Sales/Itinerary/Itinerary_list.html', {'itineraries': itineraries, 'query': query})
...
def itinerary_edit(request, pk):
    ...
    return render(request, 'Sales/Itinerary/Itinerary_edit.html', {
        'itinerary': itinerary,
        'customers': customers,
        'sales_persons': sales_persons,
...
def itinerary_view(request, pk):
    itinerary = get_object_or_404(Itinerary, pk=pk)
    return render(request, 'Sales/Itinerary/Itinerary_view.html', {'itinerary': itinerary})
...
def itinerary_delete(request, itinerary_id):
    ...
    if request.method == "POST":
        ...
        return redirect('listitinerary')  # Adjust this redirect to your actual voucher list view name
    

def send_itinerary_email(request, itinerary_id):
    itinerary = get_object_or_404(Itinerary, id=itinerary_id)
...
def send_whatsapp_itinerary(customer_display_name, destination, start_date, itinerary_id, contact_number):
...
def send_whatsapp_itinerary_message(request, itinerary_id):
    itinerary = get_object_or_404(Itinerary, id=itinerary_id)
    ...
    if itinerary.start_date:
        ...
        start_date = "Not Available"  # Or handle this case as needed  # Convert date to string
    contact_number = itinerary.customer.contact_number
    
    # Call the send_whatsapp_itinerary function
    send_whatsapp_itinerary(customer_display_name, destination, start_date, itinerary_id, contact_number)
    
    messages.success(request, 'WhatsApp itinerary message sent successfully.')
    return redirect('listitinerary')  # Adjust this to your actual itinerary list view name

def itinerary_whatsapp(request, itinerary_id):
    itinerary = get_object_or_404(Itinerary, id=itinerary_id)
