...
def saleinvoice(request):
    ...
    return render(request, 'Sales/Sale_invoice/Sale_invoice.html', {'invoices': invoices, 'query': query})
...
@login_required(login_url='admin_loginview')
def saleinvoiceview(request, invoice_id):
    invoice = get_object_or_404(Invoice, id=invoice_id)
    context = {
        'invoice': invoice
    }
    return render(request, 'Sales/Sale_invoice/Sale_invoice_view.html', context)

@login_required(login_url='admin_loginview')
def saleinvoiceedit(request, invoice_id):
    invoice = get_object_or_404(Invoice, id=invoice_id)
    ...
    return render(request, 'Sales/Sale_invoice/Sale_invoice_edit.html', context)



@login_required(login_url='admin_loginview')
def saleinvoicedelete(request, invoice_id):
    invoice = get_object_or_404(Invoice, id=invoice_id)
    if request.method == "POST":
        invoice.delete()
        messages.success(request, 'Invoice deleted successfully.')
        return redirect('saleinvoice')  # Adjust this redirect to your actual voucher list view name
    
#------------------------------------------------- Record Payment --------------------------------------

@login_required(login_url='admin_loginview')
def invoice_record_payment(request, id):
    ...
    if request.method == 'POST':
        ...
        return redirect('saleinvoiceview', invoice_id=id)

    return render(request, 'Sales/Sale_invoice/Sale_invoice_view.html', {'invoice': invoice})
...
#------------------------------------- Invoice wattsapp message ---------------------------------------

# def send_whatsapps(customer_display_name, resort_name, checkin_date, checkin_time, invoice_id, contact_number):
#     url = "https://live-mt-server.wati.io/329541/api/v1/sendTemplateMessages"

#     payload = {
#         "broadcast_name": "welcomeinvo",
#         "receivers": [
#             {
#                 "customParams": [
...
#                     {"name": "invoice_url", "value": f"https://www.sales.navigotrips.com/invoice_whattsapp/{invoice_id}/"},
#                     {"name": "resort_location", "value": contact_number}
#                 ],
#                 "whatsappNumber": "91" + contact_number

#             }
#         ],
#         "template_name": "navigo_invoice"
#     }
#     headers = {
...
def invoice_whattsapp(request, invoice_id):
    invoices = get_object_or_404(Invoice, id=invoice_id)
...
# def send_whatsapp(customer_display_name, resort_name, checkin_date, checkin_time, invoice_id, contact_number):
#     url = "https://live-mt-server.wati.io/329541/api/v1/sendTemplateMessages"
...
#     invoice_url = f"https://www.sales.navigotrips.com/invoice_email/{invoice.id}/"  # Assuming invoice id is unique

#     # Call the send_whatsapp_reminder function
#     send_whatsapp(customer_display_name, resort_name, checkin_date, checkin_time, invoice_id, contact_number)
...
#------------------------------------- Cancel Invoice wattsapp message -------------------------------------

# def send_whatsapp_invoice_delete(customer_display_name, resort_name, checkin_date, checkin_time, invoice_id, mobile):
...
#     # Call the send_whatsapp_reminder function
#     send_whatsapp_invoice_delete(customer_display_name, resort_name, checkin_date, checkin_time, invoice_id, mobile)
...
def send_whatsapp_postpone_invoice(invoice_id, new_date,new_resort_name=None):
    invoice = get_object_or_404(Invoice, id=invoice_id)
...
def send_whatsapp_postpone_invoice_message(request, invoice_id):
    invoice = get_object_or_404(Invoice, id=invoice_id)
    ...
    send_whatsapp_postpone_invoice(invoice_id, new_date)
...
def postpone_invoice(request, invoice_id):
    invoice = get_object_or_404(Invoice, id=invoice_id)
    ...
    if request.method == "POST":
        try:
            ...
            send_whatsapp_postpone_invoice(
                invoice.id, 
            ...
            return redirect('postpone_invoice', invoice_id=invoice_id)
    ...
    return render(request, 'Postpone/postpone_invoice.html', {
        'invoice': invoice,
        'customers': customers,
        'sales_persons': sales_persons,
        'resorts': resorts,
...
def send_whatsapp_cancel_invoice_message(request, invoice_id, cancellation_date):
    invoice = get_object_or_404(Invoice, id=invoice_id)
...
def cancelinvoice(request, id):
    ...
    if request.method == "POST":
        try:
            ...
            send_whatsapp_cancel_invoice_message(request, invoice.id, invoice.cancellation_date)
...
#         due_date = request.POST.get('due_date')
#         sales_person_id = request.POST.get('sales_person')
#         resort_name_id = request.POST.get('resort_name')
#         checkin_date = request.POST.get('checkin_date')
#         checkout_date = request.POST.get('checkout_date')
#         mobile = request.POST.get('mobile')
#         checkin_time = request.POST.get('checkin_time')
...
#         package_price = Decimal(request.POST.get('package_price') or '0')
#         resort_price = Decimal(request.POST.get('resort_price') or '0')
#         tax = Decimal(request.POST.get('tax') or '0')
#         recieved_price = Decimal(request.POST.get('recieved_price') or '0')

#         # Perform calculations
#         total_amount = package_price + (package_price * tax / 100)
#         pending_price = max(total_amount - recieved_price, 0)
#         profit = package_price - resort_price
#         navigo = resort_price - pending_price

#         # Convert string inputs to appropriate types if necessary
#         sales_person = CustomUser.objects.get(id=sales_person_id)
#         resort_name = Resorts.objects.get(id=resort_name_id)
#         meal_plan = MealPlans.objects.get(id=meal_plan_id)
#         account_name = BankAccount.objects.get(id=account_name_id)
...
#     latest_invoice = Invoice.objects.aggregate(Max('id'))
#     next_invoice_number = f"INV{latest_invoice['id__max'] + 1 if latest_invoice['id__max'] is not None else 1:02d}"

#     context = {
#         'sales_people': CustomUser.objects.all(),
