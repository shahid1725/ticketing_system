...
def employee_leads_report(request):
    start_date = None
    end_date = None
    leads_data = None
    ...
    if request.method == 'POST':
        ...
        if 'export_excel' in request.POST:
            ...
            headers = ['#', 'Full Name', 'Email', 'Mobile Number', 'Lead Date', 'Attended By']
            worksheet.append(headers)

            for idx, lead in enumerate(leads_data, start=1):
                formatted_lead_date = lead.lead_date.strftime('%d-%m-%Y') if lead.lead_date else ''
                row = [
                    idx, lead.full_name, lead.email, lead.mobile, formatted_lead_date
                ]
                worksheet.append(row)

            column_widths = [5, 25, 15, 25, 15, 30]
    ...
    return render(request, 'EmpReport/employee_LeadsReport.html', {'leads_data': leads_data, 'start_date': start_date, 'end_date': end_date})
...
def employee_sales_report(request):
    ...
    if 'export_excel' in request.POST:
        ...
        headers = ['No', 'Date', 'Invoice No', 'Customer', 'Checkin Date', 'Price Quoted', 'Advance Amount', 'Due Date', 'Account Number', 'Resort Name']
        worksheet.append(headers)
    ...
    return render(request, 'EmpReport/employee_Report.html', {
        'data': data,
        'employees': employees,
...
@login_required(login_url='employee_signin')
def employee_voucher_report(request):
    employees = CustomUser.objects.exclude(username='admin')
    resorts = Resorts.objects.all()
    ...
    if 'export_excel' in request.POST:
        ...
        headers = ['No', 'Date', 'Voucher No', 'Customer', 'Checkin Date', 'Price Quoted', 'Advance Amount', 'Travel', 'Account ',
                   'Resort Name']
        worksheet.append(headers)
    ...
    return render(request, 'EmpReport/Employee_VoucherReport.html', {
        'data': data,
        'employees': employees,
        'selected_employee': selected_employee,
        'resorts': resorts,
...
def employee_profit_report(request):
    employees = CustomUser.objects.exclude(username='admin')
    resorts = Resorts.objects.all()
    ...
    if 'export_excel' in request.POST:
        ...
        invoice_headers = [
            ...
            'Advance Amount', 'Total Amount', 'Balance Amount', 'Sales Person', 'Account Details', 'No of nights',
            'No of rooms'
        ]
    ...
    return render(request, 'EmpReport/employee_ProfitReport.html', {
