...
def employee_resortlist(request):
    ...
    return render(request,'EmpSales/EmpResorts/employee_Resorts_List.html',{'resorts':resorts, 'has_resorts': has_resorts})


@login_required(login_url='employee_signin')
def employee_resortadd(request):
    if request.method=='POST':
        ...
        if Resorts.objects.filter(resort_mobile=resort_mobile).exists():
            ...
            return render(request,'EmpSales/EmpResorts/employee_Resorts_Add.html')
        ...
        try:
            ...
            return redirect('employee_resortlist')
...
@login_required(login_url='employee_signin')
def employee_resortedit(request,id):
    ...
    if request.method=='POST':
        ...
        if Resorts.objects.filter(resort_mobile=resorts.resort_mobile).exclude(id=resorts.id).exists():
            messages.error(request,'A resort with this contact number already exists')
            return render(request,'EmpSales/EmpResorts/employee_Resorts_Edit.html',{'resorts':resorts})
        ...
        if len(resorts.resort_mobile) < 10:
            messages.error(request,'Contact number must be at least 10 numbers long')
            return render(request,'EmpSales/EmpResorts/employee_Resorts_Edit.html')
...
@login_required(login_url='employee_signin')
def employee_resortdelete(request,id):
    resorts=get_object_or_404(Resorts,id=id)
    if request.method=='POST':
        resorts.delete()
