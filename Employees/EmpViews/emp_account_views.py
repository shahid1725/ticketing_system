...
def employee_accountdetails(request):
    ...
    return render(request,'EmpSales/EmpAccount_Details/employee_Account_Details_list.html',{'account':account, 'has_account': has_account})

@login_required(login_url='employee_signin')
def employee_accountdetailsadd(request):
    if request.method == 'POST':
        ...
        if BankAccount.objects.filter(account_number=account_number).exists():
            ...
            return render(request,'EmpSales/EmpAccount_Details/employee_Account_Details_Add.html')
        ...
        return redirect('employee_accountdetails')
    return render(request,'EmpSales/EmpAccount_Details/employee_Account_Details_Add.html')


@login_required(login_url='employee_signin')
def employee_accountdetailsedit(request,id):
    ...
    if request.method == 'POST':
        ...
        if BankAccount.objects.filter(account_number=account.account_number).exclude(id=account.id).exists():
            messages.error(request,'An account with this account number already exists')
            return render(request,'EmpSales/EmpAccount_Details/employee_Account_Details_Edit.html',{'account':account})
...
@login_required(login_url='employee_signin')
def employee_accountdetailsdelete(request, account_id):
