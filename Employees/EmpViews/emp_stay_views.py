...
def employee_stayadd(request):
    ...
    return render(request, 'EmpPackages/employee_Stay_Add.html')

@login_required(login_url='admin_loginview')
def employee_stayedit(request, stay_id):
    ...
    return render(request, 'EmpPackages/employee_Stay_edit.html', {
        'stay': stay,
        'room_data': room_data,
    })
...
@login_required(login_url='employee_signin')
def employee_staydelete(request,id):
...
def employee_send_stay_email(request, stay_id):
    if request.method == 'POST':
        stay = get_object_or_404(Stay, id=stay_id)
        ...
        html_message = render_to_string('EmpPackages/employee_stay_email_template.html', context)
...
def employee_send_stay_whatsapp_message(request, stay_id):
    if request.method == 'POST':
        stay = get_object_or_404(Stay, id=stay_id)
